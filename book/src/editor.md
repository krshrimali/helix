...
| `detect-system-theme` | Whether to query the OS for the system theme (light/dark mode). On Windows/WSL this queries the registry, which may slow startup | `false` |
...
| `breadcrumbs` | The cursor's context path using tree-sitter (e.g., `file > ClassName > function_name`) |
